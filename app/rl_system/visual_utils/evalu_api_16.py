...
NAME_TO_ID = {name: idx for idx, name in enumerate(INTERSECTION_NAMES)}
...
# ----------------- 数据加载 -----------------
def generate_plots(selected_name,log_file='C:/TrafficMind/app/rl_system/data/output_data/tsc/sumo_colight/sumohz4x4/test/logger/model_eva.log'):
    # 将输入的路口名称转换为数字ID
    selected_ids = [NAME_TO_ID[name] for name in selected_name if name in NAME_TO_ID]
    if not selected_ids:
        raise ValueError("No valid intersection names provided.")
    ...
    for name in selected_name:
        inter_id = NAME_TO_ID[name]
        fig_i, ax_i, line_r_i, line_q_i = create_intersection_fig(name)
        intersection_figs[inter_id] = (fig_i, ax_i, line_r_i, line_q_i)

        def update_intersection(frame, inter_id=inter_id, ax=ax_i, line_r=line_r_i, line_q=line_q_i):
            idx = frame * 5
            if idx >= num_frames:
                idx = num_frames - 1
            x = list(range(idx + 1))
        ...
        anim = FuncAnimation(fig_i, update_intersection, frames=num_steps, interval=800, blit=False)
        intersection_anims[inter_id] = anim
...
if __name__ == "__main__":
    if len(sys.argv)>1:
        ...
        selected_name = INTERSECTION_NAMES
    generate_plots(selected_name=["intersection_1_1"]) #调用示例